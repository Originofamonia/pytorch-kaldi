...
def run_nn(data_name, data_set, data_end_index, fea_dict, lab_dict, arch_dict, cfg_file, processed_first,
           next_config_file):
    # This function processes the current chunk using the information in cfg_file. In parallel, the next chunk is load
    # into the CPU memory reading chunk-specific cfg file (first argument-mandatory file)
    ...
    if to_do == 'train':  # tried to change to switch, but it seems much more complicated than if.
