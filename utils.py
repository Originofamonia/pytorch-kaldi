    ...
    for data in name_data:
        ...
        for name_features in fea_names_lst[cnt]:
            if not (re.match("^[a-zA-Z0-9]*$", name_features)):
                sys.stderr.write(
                    "ERROR: features names (fea_name=) must contain only letters or numbers (no special characters as "
                    "\"_,$,..\") \n")
                sec_parse = False
                sys.exit(0)
            ...
            if strtobool(forward_norm_bool_lst[i]):
                ...
                if forward_norm_lst[i] not in lab_lst:
                    ...
                    if "ali-to-pdf" in lab_opts[lab_lst.index(forward_norm_lst[i])]:
                        ...
                        count_file_path = out_folder + '/exp_files/forward_' + forward_out_lst[i] + '_' + \
                                          forward_norm_lst[i] + '.count'
    ...
    for ep in range(N_ep):
        ...
        for tr_data in tr_data_lst:
            ...
            for ck in range(N_ck_tr):
                ...
                if not strtobool(config['batches']['increase_seq_length_train']):
